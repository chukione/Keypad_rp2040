...
#ifndef ARDUINO_ARCH_RP2040
#define _mypinMode(_pin, _mode)  do {if(_mode == INPUT_PULLUP) {pinMode(_pin, INPUT); digitalWrite(_pin, 1);} if(_mode != INPUT_PULLUP) pinMode(_pin, _mode);}while(0);
#else
#define _mypinMode(_pin, _mode)  do {pinMode(_pin,INPUT_PULLUP);}while(0);
#endif
#endif
